...
# Single path segment → skip without scanning full `parts` every time.
_SKIP_DIR_SINGLE: frozenset[str] = frozenset(d for d in _SKIP_DIRS if "\\" not in d)
# Multi-segment rules (e.g. AppData\Local\Temp) as part tuples.
_SKIP_DIR_SEQS: tuple[tuple[str, ...], ...] = tuple(
    tuple(d.split("\\")) for d in _SKIP_DIRS if "\\" in d
)

# Full-disk scan: min seconds between progress line updates (printing every dir is very slow).
_SCAN_PROGRESS_MIN_INTERVAL_S = 0.25
...
def _print_startup_banner() -> None:
    """Unicode box banner shown once when the script starts."""
    title = "Akbar - aka Aharon The Gay"
    width = 64
    pad = width - len(title)
    left = pad // 2
    right = pad - left
    mid = f"{' ' * left}{title}{' ' * right}"
    bar = "═" * width
    print(
        f"\n{_CYAN}{_BOLD}╔{bar}╗\n"
        f"║{mid}║\n"
...
def _walk_prune_skip(parent: Path, name: str) -> bool:
    """True if `parent/name` should not be traversed (fast path for os.walk pruning)."""
    if name in _SKIP_DIR_SINGLE:
        return True
    parts = parent.parts + (name,)
    for seq in _SKIP_DIR_SEQS:
        n = len(seq)
        if n > len(parts):
            continue
        ...
        for root in roots:
            try:
                for dirpath, dirnames, filenames in os.walk(
                    ...
                    dirnames[:] = [d for d in dirnames if not _walk_prune_skip(current, d)]

                    dirs_scanned += 1
                    now = time.monotonic()
                    if now - last_progress_t >= _SCAN_PROGRESS_MIN_INTERVAL_S:
                        last_progress_t = now
                        print(
                    ...
                    # Only build a set when we need O(1) sibling .json lookup.
                    name_set: set[str] | None = None
                    for fname in filenames:
                        fl = fname.lower()
                        if fl.endswith(".session"):
                            if name_set is None:
                            ...
                            seen_stems.add(stem)
                            jname = stem + ".json"
                            json_p = (current / jname) if jname in name_set else None
                            ...
                            archives_scanned += 1
                            _scan_zip(current / fname, staging_dir, seen_stems, found)
                        elif _RAR_AVAILABLE and fl.endswith(".rar"):
                            archives_scanned += 1
                            _scan_rar(current / fname, staging_dir, seen_stems, found)
    ...
    try:
        with zipfile.ZipFile(archive_path, "r") as zf:
            ...
            for sname in session_names:
                ...
                if json_key_full in names_set:
                    actual_json = json_key_full
                elif json_key_flat in names_set:
                    actual_json = json_key_flat
                else:
                    actual_json = None
...
async def run_audit(
    ...
    async def process_one(idx: int, fs: FoundSession) -> None:
        ...
        async with semaphore:
            ...
            json_pc_path = str(fs.json_path.resolve()) if fs.json_path else ""
            tdata_pc_path = str(fs.tdata_path.resolve()) if fs.tdata_path else ""
            archive_pc_path = str(fs.archive_path.resolve()) if fs.archive_path else ""
                ...
                if result["ok"]:
                    ...
                    for g in groups:
                        all_rows.append({
                            ...
                            "Role":                    g["role"],
                            "JSON_File_Path":          json_pc_path,
                            "TData_Folder_Path":       tdata_pc_path,
                            "Archive_File_Path":       archive_pc_path,
                        })
                else:
                    error_count += 1
