...
from nexus.core.flight_mode import FlightModeEngine
from nexus.services.sentinel import (
    redis_has_windows_worker_key,
    scan_cluster_heartbeats,
    summarise_swarm_peers_from_list,
    windows_worker_heartbeat_online,
)
...
class SentinelEngine:
    ...
    async def _check_worker_heartbeats(self) -> None:
        try:
            heartbeats = await scan_cluster_heartbeats(self._redis)
            explicit_key = await redis_has_windows_worker_key(self._redis)
            # Configured Windows heartbeat key OR any Windows-leaning worker (excludes Linux anchor IP).
            windows_online = windows_worker_heartbeat_online(
                heartbeats,
                explicit_redis_key_hit=explicit_key,
            )

            summary = summarise_swarm_peers_from_list(heartbeats)
            if summary.get("expected_ips_missing"):
                log.debug(
                    "sentinel_swarm_anchor_ips_missing",
                    missing=summary["expected_ips_missing"],
                    seen_ips=summary["local_ips"],
