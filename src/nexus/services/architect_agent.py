...
MODULES_ROOT = NEXUS_ROOT / "src" / "nexus" / "agents" / "modules"
...
class ArchitectAgent:
    ...
    async def _audit_cycle(self) -> dict[str, Any]:
        ...
        # --- 1. Scan the src/nexus package ---
        nexus_findings = await asyncio.get_event_loop().run_in_executor(
            None,
            lambda: _scan_directory(NEXUS_ROOT / "src" / "nexus", max_files=300),
        )

        # --- 2. Scan tools/ ---
        scripts_findings = await asyncio.get_event_loop().run_in_executor(
            None,
            lambda: _scan_directory(NEXUS_ROOT / "tools", max_files=50),
        )
