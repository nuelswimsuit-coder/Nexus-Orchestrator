...
class AutonomousOrchestrator:
    ...
    def __init__(
        self,
        dispatcher: Any,   # nexus.core.dispatcher.Dispatcher
    ...
    async def run_loop(self, interval_seconds: int = 300) -> None:
        ...
        while self._running:
            try:
                ...
                log.exception("orchestrator_cycle_error", error=str(exc))
                try:
                    await self._log_entry("error", f"Cycle error: {exc}", {})
                except Exception:
                    log.warning("orchestrator_log_entry_failed_after_cycle_error")
    ...
    async def _pulse_log(self, ctx: Any, fin: dict) -> None:
        ...
        try:
            from nexus.services.explorer import get_budget_widget_data
    ...
    async def _check_stuck(self, action: ScoredAction, gap: int) -> None:
        ...
        dashboard_url = "http://localhost:8001/nexus-os"
        try:
