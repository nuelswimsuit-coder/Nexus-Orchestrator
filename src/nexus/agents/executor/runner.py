...
async def run_task(
    ...
    effective_params["project_id"] = task.project_id
    # Inject task_id so handlers can open the checkpoint store with the correct key.
    effective_params["__task_id__"] = task.task_id
    if "task_id" not in effective_params:
        effective_params["task_id"] = task.task_id
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if remaining > 0:
                ...
                # Persist the failure in the checkpoint store so the next run
                # knows this task crashed and can resume from the last safe point.
                try:
                    cp = CheckpointStore(task.task_id)
                    cp.mark_failed("__task__", last_error)
                except Exception:
                    pass

    # All retries exhausted — also record in the telefix tasks table
    try:
        from nexus.shared.db_util import record_task_failure
        record_task_failure(
            task_id=task.task_id,
            task_type=task.task_type,
