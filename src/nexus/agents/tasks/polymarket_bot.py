...
from nexus.agents.trading.poly_bot_state import (
    BTC_TTL_S,
    PNL_TTL_S,
    POLY_BOT_BTC_KEY,
...
async def _run_tick(redis: Any, params: dict[str, Any]) -> dict[str, Any]:
    # AGGRESSIVE TEST MODE defaults: wide proximity, high ceiling, safe bet, low liquidity bar
    max_bet = float(params.get("max_bet_usd", 5.0))
    yes_ceiling = float(params.get("yes_ceiling", 0.95))
    proximity = float(params.get("proximity_pct", 0.05))
    stop_loss_pct = float(params.get("stop_loss_pct", 0.20))
    min_liquidity_usd = float(params.get("min_liquidity_usd", 10.0))

    print(
        f"\033[1;34m🔍 [SCANNING] Checking Polymarket for BTC opportunities...\033[0m",
        flush=True,
    )
    ...
    if (
        ...
        if near and yes_px < yes_ceiling:
            ...
            # ── SIGNAL DETECTED banner — fires BEFORE the async buy task ──
            ...
            print(
                f"{_CYAN_BOLD}"
                f"{'=' * 60}\n"
                f"🔥 [SIGNAL DETECTED] - THRESHOLD PASSED! EXECUTING ORDER...\n"
                f"   BTC=${btc:,.2f}  |  Strike=${strike:,.2f}  |  YES={yes_px:.4f}  |  Budget=${max_bet:.2f}\n"
                f"{'=' * 60}"
                f"{_RESET}",
                flush=True,
            )
            shares_est = round(max_bet / yes_px, 2) if yes_px > 0 else 0
            print(
                f"{_GREEN_BOLD}"
                f"[NEXUS BUY] >>> POLYMARKET ORDER FIRING <<<\n"
                f"  Market  : {mq}\n"
                f"  BTC Spot: ${btc:,.2f}  |  Strike: ${strike:,.2f}\n"
                f"  Side    : YES  |  Price: ${yes_px:.4f}  |  Budget: ${max_bet:.2f}\n"
                f"  Est. Shares: {shares_est}  |  Token: {token_id[:16]}…\n"
                f"  Tick Size: {tick}"
                f"{_RESET}",
                flush=True,
            )
            log.info(
                "polymarket_bot_buy_firing",
                market_question=mq,
                btc_spot=btc,
                strike=strike,
                yes_price=yes_px,
            ...
            # Wrap the buy in create_task so the UI loop is never blocked on
            # order confirmation — the task resolves off the hot path.
            async def _execute_buy() -> None:
                nonlocal open_pos, last_action, detail
                try:
                    ...
                    print(
                        f"\033[1;31m[NEXUS BUY] BLOCKED: {exc}\033[0m",
                        flush=True,
                    )
                    return
                if buy_res.success:
                    ...
                    print(
                        f"{_GREEN_BOLD}[NEXUS BUY] SUCCESS: {detail}{_RESET}",
                        flush=True,
                    )
                else:
                    ...
                    print(
                        f"\033[1;31m[NEXUS BUY] FAILED: {detail}\033[0m",
                        flush=True,
                    )

            # Await the task directly within the tick — create_task schedules it
            # on the event loop so it doesn't block any concurrent coroutines.
            buy_task = asyncio.create_task(_execute_buy(), name="poly-buy-order")
            await buy_task
...
@registry.register("research_new_market")
async def research_new_market(parameters: dict[str, Any]) -> dict[str, Any]:
    """
    Autonomous objective when the core supervisor injects work on an idle ARQ queue
    (``scripts/nexus_core.py`` with ``--turbo-boost``): sample active Polymarket Gamma markets.
    """
    limit = int(parameters.get("limit", 40))
    limit = min(max(limit, 1), 100)
    params = {"limit": str(limit), "active": "true"}
    async with httpx.AsyncClient(timeout=30.0) as client:
        r = await client.get(POLYMARKET_GAMMA_URL, params=params)
        r.raise_for_status()
        data = r.json()
    n = len(data) if isinstance(data, list) else 0
    log.info("research_new_market_complete", markets=n)
    return {"markets_sampled": n, "autonomous": True}
...
async def polymarket_bot_tick(parameters: dict[str, Any]) -> dict[str, Any]:
    ...
    The CLOB heartbeat is written unconditionally so the dashboard stays green
    even when the private key is absent (trading is skipped but heartbeat lives).
    """
    ...
    # Warn (not raise) when the private key is missing so the heartbeat loop
    # keeps running even in read-only / paper-trading environments.
    _active_key = (
        os.getenv("POLYMARKET_RELAYER_KEY")
    ...
    if not _active_key:
        log.warning(
            "polymarket_bot_no_private_key",
            hint="Set POLYMARKET_RELAYER_KEY in .env to enable live trading; heartbeat continues.",
        )
    else:
        log.info(
            "polymarket_bot_live_key_active",
            key_prefix=_active_key[:6],
            detail="LIVE trading enabled — real CLOB orders will be placed",
        )
        print(
            f"\033[1;32m✅ [POLY-BOT] LIVE KEY ACTIVE ({_active_key[:6]}…) "
            "— real CLOB orders enabled\033[0m",
            flush=True,
        )
