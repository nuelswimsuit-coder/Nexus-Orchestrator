...
def _build_account_map_payload(
    client: Any,
    session_label: str,
    meta_path: str,
    ...
    try:
        ...
        return {
            ...
            "meta_path": meta_path or "(vault_string_session)",
            "user_id": me.id,
            "phone": phone_str,
...
def _map_one_session_string(
    ...
    return _build_account_map_payload(
        client, session_label, "", premium_scan_limit=premium_scan_limit
    )
...
def _map_one_session(
    ...
    if not client.is_user_authorized():
        ...
        raise PermissionError(f"Session not authorized: {session_label}")
    return _build_account_map_payload(
        client, session_label, str(meta_json), premium_scan_limit=premium_scan_limit
    )
...
def _run_map_job(
    ...
    output_path: Path,
    *,
    vault_sessions: list[dict[str, Any]] | None = None,
    ...
    use_vault = vault_sessions is not None
    meta_files: list[Path] = []
    if not use_vault:
        meta_files = discover_session_meta_json_files(staged_dir)
    if use_vault and not vault_sessions:
        return {
            "status": "no_sessions",
    ...
    if not use_vault and not meta_files:
        return {
            "status": "no_sessions",
            "staged_dir": str(staged_dir),
            "sessions": [],
    ...
    n_sessions = len(vault_sessions or meta_files)
    for i in range(n_sessions):
        ...
        try:
            ...
            if use_vault:
                cred = (vault_sessions or [])[i]
                stem = str(cred.get("session_stem") or f"vault_{i}")
                meta_disp = "(vault)"
            else:
                meta_path = meta_files[i]
                stem = meta_path.stem
                meta_disp = str(meta_path)
            log.error(
...
async def map_accounts(parameters: dict[str, Any]) -> dict[str, Any]:
    ...
    vault_sessions: list[dict[str, Any]] | None
    if "__vault_string_sessions__" in parameters:
        vault_sessions = list(vault_raw) if isinstance(vault_raw, list) else []
    else:
        vault_sessions = None
