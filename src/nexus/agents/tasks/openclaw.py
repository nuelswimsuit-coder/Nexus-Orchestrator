...
# Polymarket CLOB order book (Bitcoin Up/Down) for cross-venue stack
OPENCLAW_POLY_ORDERBOOK_KEY = "nexus:openclaw:polymarket_orderbook"
OPENCLAW_POLY_ORDERBOOK_TTL = 900  # 15 minutes
...
async def _fetch_polymarket_orderbook_for_openclaw(redis_client: Any) -> None:
    """
    Pull Polymarket **order book** (not just Gamma last prices) for the active
    BTC price-action market and mirror it to Redis for dashboards / fusion.
    """
    raw = os.getenv("OPENCLAW_FETCH_POLY_ORDERBOOK", "1").strip().lower()
    ...
    try:
        ...
        log.debug("openclaw_poly_ob_import_failed", error=str(exc))
        return
    try:
        async with httpx.AsyncClient(timeout=15.0) as c:
            m = await fetch_polymarket_btc_price_action_contract(c)
            if not m.get("market_found"):
                return
            toks = m.get("clob_token_ids") or []
            if not toks:
                return
            book = await fetch_polymarket_clob_orderbook(toks[0], c)
        summary = summarize_polymarket_orderbook(book)
        payload = {
            "updated_at": datetime.now(timezone.utc).isoformat(),
            "market_id": m.get("market_id"),
            "market_question": m.get("market_question"),
            "up_token_id": toks[0],
            "gamma_up_price": m.get("yes_price"),
            "orderbook": summary,
        }
        await redis_client.set(
            OPENCLAW_POLY_ORDERBOOK_KEY,
            json.dumps(payload),
            ex=OPENCLAW_POLY_ORDERBOOK_TTL,
        )
        log.info("openclaw_polymarket_orderbook_saved", market_id=m.get("market_id"))
    except Exception as exc:
        log.warning("openclaw_polymarket_orderbook_failed", error=str(exc))
...
async def browser_scrape(parameters: dict[str, Any]) -> dict[str, Any]:
    ...
    redis_client = parameters.get("__redis__")
    if redis_client is not None:
        await _fetch_polymarket_orderbook_for_openclaw(redis_client)
