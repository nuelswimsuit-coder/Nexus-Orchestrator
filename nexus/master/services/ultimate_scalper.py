...
* Live: ``POLYMARKET_RELAYER_KEY`` / ``NEXUS_POLY_PRIVATE_KEY`` via ``wallet_manager``;
  30% drawdown brake + Telegram (see ``nexus.trading.wallet_manager``).
...
from nexus.master.services.poly_5m_scalper import POLY_EVENT_ID, fetch_poly5m_market
from nexus.shared.kill_switch import KILL_SWITCH_SCALPER_HALT_KEY
from nexus.master.services.strategy_brain import (
...
from nexus.trading.wallet_manager import REDIS_BRAKE_KEY, evaluate_real_balance_safety_brake
from nexus.worker.tasks.openclaw import OPENCLAW_NEWS_SENTIMENT_KEY
from nexus.worker.tasks.prediction import BINANCE_VELOCITY_KEY, fetch_binance_data
...
SIMULATION_MODE: bool = (
    os.getenv("POLY_SCALPER_SIMULATION_MODE", "true").strip().lower() in ("1", "true", "yes", "on")
)


def simulation_mode_from_env() -> bool:
    if os.getenv("POLY_SCALPER_SIMULATION_MODE") is None:
        return SIMULATION_MODE
    return os.getenv("POLY_SCALPER_SIMULATION_MODE", "true").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
...
async def read_simulation_mode(redis: Any) -> bool:
    ...
    return simulation_mode_from_env()
