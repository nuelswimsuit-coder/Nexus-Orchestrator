...
momentum, race-to-1000%% UI), see ``nexus.master.services.ultimate_scalper``.

Coordinates:
  • Worker-published Binance websocket feed (``nexus:poly5m:btc_feed``, task in
    ``nexus.worker.tasks.poly5m_velocity`` when ``POLY5M_VELOCITY_FEED=1``).
...
from nexus.worker.tasks.poly5m_velocity import POLY5M_BTC_FEED_KEY
...
def simulation_mode_from_env() -> bool:
    v = (os.getenv("POLY_SCALPER_SIMULATION_MODE") or "true").strip().lower()
...
async def run_poly_scalper_loop(redis: Any) -> None:
    ...
    from nexus.master.services.ultimate_scalper import run_poly_scalper_loop as _ultimate_loop
